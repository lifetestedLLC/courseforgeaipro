...
- **Local development** (localhost:3000) works ✅
- **Production** (www.courseforgeai.org) fails ❌
...
6. Click **"Create"**
7. ✅ Vercel automatically sets the `DATABASE_URL` for you!
...
2. Click **three dots (...)** on the latest deployment → **"Redeploy"**
...
In Vercel → Settings → Environment Variables, make sure these are set:

- ✅ **DATABASE_URL** = `postgresql://...` (NOT `prisma+postgres://localhost:...`)
- ✅ **NEXTAUTH_URL** = `https://courseforgeai.org` or `https://www.courseforgeai.org`
...
Common issues:
- **"DATABASE_URL not found"** → Make sure you saved it in Vercel and redeployed
- **"Tables don't exist"** → Run Step 2 again (database migrations)
- **"Connection timeout"** → Your database firewall might be blocking Vercel
...
1. ✅ Create production database (Vercel Postgres, Supabase, or Neon)
2. ✅ Set DATABASE_URL in Vercel environment variables
3. ✅ Run database migrations (`npx prisma db push`)
4. ✅ Create admin user (`npm run db:seed`)
5. ✅ Redeploy on Vercel
6. ✅ Test on www.courseforgeai.org
